...
echo Command: "%PYTHON_CMD%" examples\fit_q0_obs_map.py "%OBS_FITS_PATH%" "%MODEL_H5_PATH%" --ebtel-path "%EBTEL_PATH%" --q0-min 0.01 --q0-max 2.5 --target-metric chi2 --psf-bmaj-arcsec 5.77 --psf-bmin-arcsec 5.77 --psf-bpa-deg -17.5 --psf-ref-frequency-ghz 17.0 --psf-scale-inverse-frequency --artifacts-dir "%ARTIFACTS_DIR%" --show-plot %EXTRA_ARGS%
if "%DRY_RUN%"=="1" exit /b 0

pushd "%PYCHMP_REPO%"
"%PYTHON_CMD%" examples\fit_q0_obs_map.py "%OBS_FITS_PATH%" "%MODEL_H5_PATH%" --ebtel-path "%EBTEL_PATH%" --q0-min 0.01 --q0-max 2.5 --target-metric chi2 --psf-bmaj-arcsec 5.77 --psf-bmin-arcsec 5.77 --psf-bpa-deg -17.5 --psf-ref-frequency-ghz 17.0 --psf-scale-inverse-frequency --artifacts-dir "%ARTIFACTS_DIR%" --show-plot %EXTRA_ARGS%
