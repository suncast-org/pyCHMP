...
def test_load_obs_map_euv_external_fits_converts_integrated_dn(tmp_path) -> None:
...
def test_load_obs_map_model_refmap_converts_using_source_path_attr(tmp_path) -> None:
    source_fits = tmp_path / "source_171.fits"
    source_data = np.full((2, 2), 80.0, dtype=np.float32)
    source_header = fits.Header()
    source_header["EXPTIME"] = 4.0
    fits.PrimaryHDU(data=source_data, header=source_header).writeto(source_fits)

    model_h5 = tmp_path / "model.h5"
    embedded = np.full((2, 2), 160.0, dtype=np.float32)
    header = fits.Header()
    ...
    with h5py.File(model_h5, "w") as h5f:
        ...
        group.attrs["source_path"] = str(source_fits)
